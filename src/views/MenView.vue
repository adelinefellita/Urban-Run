...
export default {
    ...
    methods: {
        ...
        searchShoes() {
            if (this.search.trim() === '') {
                axios.get('http://localhost:3000/men-shoes')
                    .then((response) => {
                        this.setProducts(response.data);
                    })
                    .catch((error) => {
                        console.log("Error fetching all men's shoes:", error);
                    });
            } else {
                axios.get('http://localhost:3000/men-shoes')
                    .then((response) => {
                    ...
                    .catch((error) => {
                        console.log("Error searching men's shoes:", error);
                    });
            }
        }
    },
    mounted() {
        axios.get('http://localhost:3000/men-shoes')
            ...
            .catch((error) => {
                console.log("Error fetching men's shoes:", error);
            });
    }
}
</script>
